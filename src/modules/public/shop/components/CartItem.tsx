...
import { resetCart, toogleQuantity } from '../../../../store/purchaseCartSlice';
    ...
    <>
        <Drawer drawer={drawer} closeDrawer={() => closeDrawer()}>
            ...
            <div className='grow overflow-y-scroll py-2 w-full'>
                ...
                {
                cartItems?.map((item) => (
                    item.quantity > 0 && <div key={item.id} className='flex py-2 px-3 user-item-shadow rounded-md w-11/12 mx-auto my-3'>
                        ...
                        <div className='w-8 flex flex-col items-center justify-between py-1'>
                            ...
                            <button onClick={() => dispatch(toogleQuantity({id:item.id,type:"REMOVE"}))}><FiMinus /></button>
                        </div>
                    </div>
                ))
                }
